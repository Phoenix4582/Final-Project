...
def initArgs():
    ...
    parser.add_argument("--dest", default='input/ground-truth', type = str, help = 'parent directory of txts')
    parser.add_argument("--target", default='input/images-optional', type = str, help = 'optional, parent directory for storing copied ground truths')
...
def writeFile(txt, boxes):
    for box in boxes:
        txt.write(f"cow {box[0]} {box[1]} {box[2]} {box[3]}\n")
...
def main():
    ...
    if args.src:
        ...
        for img, mask in zip(img_dirs, mask_dirs):
            ...
            f = open(os.path.join(dest, txt_name), 'w+')
